...
class Model_orders extends CI_Model
{
	...
	public function update($id)
	{
		if($id) {
            ...
            if( $this->input->post('delboy')){
			$data = array(
	    		'paid_status' => $this->input->post('paid_status'),
	    		'delboy'=>$this->input->post('delboy'),
	    		'status'=>'1'
	    	);
	    	$del = array('work_assign'=>'1');
	    	$this->db->where('id',$this->input->post('delboy'));
	    	$this->db->update('delboy',$del);
            }else{
               $data = array(
	    		'paid_status' => $this->input->post('paid_status')
	    		);
...
// 			$delete_item = $this->db->delete('orders_item');
	...
	public function order_item($data)
	{
	...
	public function delete_usercart($userId)
	{
	    $this->db->where('user_id',$userId);
	...
	{
	    $this->db->select('orders_item.*,products.*');
	    $this->db->from('orders_item');
	    $this->db->join('products','orders_item.product_id=products.id','inner');
	    $this->db->where('orders_item.order_id',$id);
	    $query = $this->db->get();
	    return $query->result();
	}
	
	public function emporders($id)
	{
	    $this->db->select('*');
		$this->db->from('orders');
		$this->db->where('delboy',$id);
		$this->db->where('status','1');
		$query = $this->db->get();
		return $query->result();
	}
	
	public function emp_orders_history($id)
	{
	    $this->db->select('*');
		$this->db->from('orders');
		$this->db->where('delboy',$id);
	...
	public function UpdateOrderSts($id){
		$data = array('status' => 1);
	...
	public function change_all_notification_status(){	
			$data = array('notification_status' => 1);	
			$update = $this->db->update('orders', $data);	
			return ($update == true) ? true : false;	
	}
	...
	{
	    $this->db->where('id',$id);
	    $delete = $this->db->delete('client_address');
	    return ($delete==true)?true:false;
	}

	
	public function get_all_orders_notification(){
		$this->db->select('*');
		$this->db->from('orders');
	...
	public function get_topten_orders_notification()
	{
		$this->db->select('*');
		$this->db->from('orders');
		$this->db->where('notification_status','0');
